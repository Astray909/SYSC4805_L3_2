...
void setup() 
{
  ...
  while (millis() < myTime + 10000)
  {
    ...
    snprintf(report, sizeof(report), "min: {%.0f, %.0f, %.0f} max: {%.0f, %.0f, %.0f}", mag_min.x, mag_min.y, mag_min.z, mag_max.x, mag_max.y, mag_max.z);
...
void loop() {
  ...
  snprintf(report, sizeof(report), "Mag: X-axis: %.3f, Y-axis: %.3f, Z-axis: %.3f .", current.x, current.y, current.z);
  Serial.println(report);
  delay(500);
}